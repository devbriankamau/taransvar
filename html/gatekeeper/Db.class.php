<?php 
//NOTE! See CDb::construct for login info...
...
class CDb extends CBasic
{
    ...
    public function __construct()
    {
        ...
        {
		    if (file_exists("getDbName.php"))
		    {   
                include "getDbName.php";
                global $szGlobalDBHost, $szGlobalDBDBName, $szGlobalDBUserName, $szGlobalDBPass;
                getDbName();

                $szDBHost = $szGlobalDBHost; //= "cyberrehab.org.mysql";
                $szDBDBName = $szGlobalDBDBName; // = "cyberrehab_org";
                $szDBUserName = $szGlobalDBUserName;// = "cyberrehab_org";
                $szDBPass = $szGlobalDBPass;// = "yzPVwtnd2k3YUtr6743AwssDEWDD";		
            }
            else
		        if (file_exists("on_server_test.php"))
		        {
                    $szDBHost = "cyberrehab.org.mysql";
                    $szDBDBName = "cyberrehab_org";
                    $szDBUserName = "cyberrehab_org";
                    $szDBPass = "yzPVwtnd2k3YUtr6743AwssDEWDD";		
                }
		        else
		        {
		            $szDBHost = "localhost";
                    $szDBDBName = "politics";
                    $szDBUserName = "polWwwUsr5643";
                    $szDBPass = "retyqwc65io2vb1u8";
		            //$szDBDBName = "testen";
...
public function memberFunctionIsDefined($szFunc) 
{
    ...
    public function prepareAndExecute($szSQL, &$cParamArr, $bPrintError = true)
    {
        if (false)//$this->bBigSelects) //200304
...
*/        
    ...
    public function execute($szSQL, $cParamArr, $bPrintError = true)
    {
        $this->prepareAndExecute($szSQL, $cParamArr, $bPrintError);
    }
...
static function convertWikies()
{
    ...
    public function fetchNextLike($szFlds, $szTbl, $szFld, $szTag, $szNeedle, $bBigSelects = false)
    {
        if (!$this->pStmt)
        {
            if ($bBigSelects || $this->bBigSelects)
                //NOTE! This is never being tured off again... Didn't check if that's a problem..
                $this->dbh->query("SET SQL_BIG_SELECTS=1");
          $szSQL = "select $szFlds from $szTbl where $szFld like $szTag";
            $this->pStmt = $this->dbh->prepare($szSQL);
/*
...
*/
            $this->pStmt->bindValue($szTag, $szNeedle, PDO::PARAM_STR);
