...
//class CSystem extends CDebug
{
    ...
    //call getSystem()->includeInfo(CSystem::INFO_CHATS) to check if set...
    const INFO_RSS = 0x1;
    const INFO_CHATS = 0x2;
    const INFO_PHONEBOOK = 0x4;
    const INFO_WALL = 0x8;
    const INFO_TIMERS = 0x10;
    const INFO_GROUPS = 0x20;
    const INFO_COMMUNITY = 0x40;
    const INFO_EMAIL_ENABLED = 0x80;
    const INFO_MODULES = 0x100;
...
{
    if (in_array($szFunction, array("initsmall","menu")))   //160209: Added "menu"... 
        return false;
    else
...
public function updateMenuShortcuts(&$cMenuArray)
{
    ...
    //Load modules.
    //asdfasdf
    if (getSystem()->includeInfo(CSystem::INFO_MODULES))
        CModule::loadModules();                    
}        
...
public function updateLeftMenuSection($szMenuSection)
{
    ...
    {
        case "groups":
            if (getSystem()->includeInfo(CSystem::INFO_GROUPS))
            {
...
public function includeInfo($nWhat)
{
    switch ($nWhat)         
    {
        case CSystem::INFO_CHATS:
            if ($_SERVER["SERVER_NAME"] == "192.168.1.103")
                return false;   //Error msg bcoz not using BDO db connection...
        ...
        case CSystem::INFO_GROUPS:
        case CSystem::INFO_COMMUNITY:
    }
    return true;
}
...
public function ajaxHandled($szClass, $szFunc, $nOverrideMayCal = 0)
{
    ...
    //CXmlCommand::addToInnerHtml("sysLog","","Class: ".$szClassName." (".$szClass.")","top");
 ...
 //   print "debug|<h1>Class name: $szClassName</h1>";
    ...
    {
    //CXmlCommand::addToInnerHtml("sysLog","","Class exists","top");
        if (!myId() && method_exists($szClassName, "handleAjaxRequest"))
        {     
           //CXmlCommand::addToInnerHtml("sysLog","","Not logged in and class exists","top");
	   //return; 170130
            ...
            if (!method_exists($szClassName, "functionRequiresLogin"))
            {
           	...
           	//CXmlCommand::addToInnerHtml("sysLog","","Function requires login","top");
                return;
            }

           //CXmlCommand::addToInnerHtml("sysLog","","About to call obj->functionRequiresLogin()","top");
            if ($nOverrideMayCal == 0)
                $bMayCall = !$pObj->functionRequiresLogin($szFunc);
            else
                $bMayCall = ($nOverrideMayCal == 1);
                
           //CXmlCommand::addToInnerHtml("sysLog","","after called", "top");
            if (!$bMayCall)
            {
                //CXmlCommand::addToInnerHtml("sysLog","","may not call", "top");
                $szExtra = (isOy()?"($szFunc) ($szClassName)":"");
                CXmlCommand::alert("You are not logged in and can't access this function $szExtra");
                CXmlCommand::addToInnerHtml("sysLog","","ALERTED", "top");
//	return;
                reportHacking("Not logged in but tried to access $szClassName : $szFunc");
//	return; //170130 - May be required!
	        //return;
                //getSystem()->login(); 
                CLogin::logout();
                return true;
...
static function handleParams()
{
    ...
    {
        case "login":
            $pSystem->addAjaxRequest("initScreen&c=menu");
            break;
...
{
    global $szServerOperationWarning;
    
    if (!strlen($szServerOperationWarning))
    {
        CSystem::handleParams();
        $szParams = getParmString();
        global $pSystem;
        
        if (strlen($szParams) || isset($_GET["func"]))
        {
            $szDefaultFunc = (isset($_GET["c"])||isset($_GET["f"])||isset($_GET["func"])?"0":"1");
            
            if (isset($_GET["c"]))
                $szParams = "c=".get("c")."&".$szParams;
            else
                $szParams = "c=cmdLine&".$szParams;
            ...
            $pSystem->addAjaxRequest($szFunc."&".$szParams);
        }
        else //140213: Before always called initScreen.. overriding cmd line params.. May have missed something by not always calling...
            $szDefaultFunc = "1";

        //def=[0|1] is read by CMenu::initScreen()
        $pSystem->addAjaxRequest("initScreen&c=menu&def=$szDefaultFunc");
            
        $pSystem->postAjaxRequests(true); //true = $bForcePrint
    }
}
...
{
    return true;    //Redefine and return false if you want to build the screen from scratch.. nothing is displayed but all libraries are loaded..
}

public function quitAfterScripts()
{
    return false;   //Redefine to build the screen but keep the standard scripts...
}

public function initialTimer()
{
...
{
	//NOTE! If not getting here... check getSystem()->includeInfo() and System setup table..(and if redefined....)
//	CXmlCommand::setInnerHtml("pingStat","","in CSystem::timer() now"); //170129
	// NOTE! Getting here for all users... alternative: redefine cronJobMinute($szCategory)
}

public function handleCmdLine()
{
    //NOTE! Not sure if should get here.... Implemented for CSystemEHealth
}

public function isPrintMode()
{
    //Some functions are different if in print mode (not displaying menu and normal divs..)
    //First used when displaying calender entry for eHealch module...
    //How to implement: 
    //      - Can't use session and can't use command line params (ajax and may have two windows open..)
    //      - js: requestData should add json array with some mode vars.. to be checked here...
    $szModJson = get("modjson");
    if (strlen($szModJson))
    {
        $cModArr = json_decode($szModJson, $bAssoc = true);
        if (isset($cModArr["printMode"]))
            return $cModArr["printMode"] +0;
    }
    return false;
}
...
{
    //Fields: class, script, more, txt
    return array(
        //<div class="mnu_top" onclick="menu('home','c=menu','')"><?php print getTxt("Home"); ?--></div>
        array("class"=>"mnu_top","script"=>"menu('home','c=menu','')", "more"=>"", "txt"=>getTxt("Home"))

        //<div id="mnuWall" class="mnu_top" class="mnu_top" onclick="menu('wall','c=menu','wall')"><?php print getTxt("Wall"); ?--></div>
        ,array("class"=>"mnu_top","script"=>"menu('wall','c=menu','')", "more"=>'id="mnuWall"', "txt"=>getTxt("Wall"))

        //<div class="mnu_top" onclick="menu('phonebook','c=menu','phonebook')"><?php print getTxt("Phone Book"); ?--></div>
        ,array("class"=>"mnu_top","script"=>"menu('phonebook','c=menu','')", "more"=>'', "txt"=>getTxt("Phone Book"))
        
        //<div class="mnu_top" onclick="menu('sys:profile','', 'rightbox')"><?php print getTxt("Profile"); ?-></div>
        ,array("class"=>"mnu_top","script"=>"menu('sys:profile','','rightbox')", "more"=>'', "txt"=>getTxt("Profile"))
        
        //<?php /* 131218 Disabled bcoz not finalized. Should test $_GLOBAL_CHAT_MODUL_ENABLED         ... <div onclick="menu('chat','c=menu', 'content')">Chat</div>*/ ?->

        //<div class="mnu_top" onclick="mnu1('help')"><?php print getTxt("Help"); ?-></div>
        ,array("class"=>"mnu_top","script"=>"mnu1('help')", "more"=>'', "txt"=>getTxt("Help"))

        //<div class="mnu_top" onclick="menu('logout','c=menu', 'rightbox')"><?php print getTxt("Logout"); ?-></div>
        ,array("class"=>"mnu_top","script"=>"menu('logout','c=menu','rightbox')", "more"=>'', "txt"=>getTxt("Logout"))
        );
}
