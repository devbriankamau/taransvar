...
{
	$conn = getConnection();
	$sql = "SELECT trafficId, inet_ntoa(T.ipFrom) as ipFrom, inet_ntoa(T.ipTo) as ipTo, T.whoIsId, CAST(isLan AS UNSIGNED) as isLan, name, portFrom, portTo, created, count from traffic T left outer join whoIs W on W.whoIsId = T.whoIsId order by trafficId desc limit 50";
	...
	if ($result->num_rows > 0) 
	{
		...
		while($row = $result->fetch_assoc()) 
		{
			$szName = ($row["isLan"] ? '<font color="gray">LAN traffic</font>' : $row["name"]);
	    		print '<tr id="tr'.$row["trafficId"].'"><td>'.$row["ipFrom"]. "</td><td>".$szName."</td><td>".$row["portFrom"]."</td><td>".$row["portTo"]."</td><td>".$row["created"]."</td><td>".$row["count"]."</td>";
